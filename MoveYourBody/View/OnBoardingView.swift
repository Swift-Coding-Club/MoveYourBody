...
//첫 실행 시에만 보여줄 온보딩 스크린으로, 아직 미적용 상태입니다.
import SwiftUI
enum Walkthrough{
...
struct WalkthroughScreen: View{
    ...
    var body: some View{
        ZStack{
            if currentPage == 1 {
                ScreenView(image: "figure.run", title: "운동 설정에서 하고 싶지 않은 동작이나 \n운동 시간을 설정할 수 있습니다.").transition(.scale)}
                
            if currentPage == 2{
                ScreenView(image: "gearshape.fill", title: "알림 설정에서 푸시 알림을 설정할 수 있습니다. \n 매일 운동하고 싶은 시간에 알림을 맞춰보세요").transition(.scale)}
            if currentPage == 3{
